        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mb-20">
          ...
          <div className="space-y-10">
            ...
            <div className="flex space-x-4">
              {[
                { Icon: Icons.Facebook, url: CONTACT_INFO.socialLinks?.facebook },
                { Icon: Icons.Twitter, url: CONTACT_INFO.socialLinks?.twitter },
                { Icon: Icons.Linkedin, url: CONTACT_INFO.socialLinks?.linkedin },
                { Icon: Icons.Instagram, url: CONTACT_INFO.socialLinks?.instagram },
                { Icon: Icons.Youtube, url: CONTACT_INFO.socialLinks?.youtube },
              ].filter(link => link.url).map((item, i) => (
                <a 
                  key={i} 
                  href={item.url} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="w-11 h-11 rounded-full bg-white/5 flex items-center justify-center hover:bg-brand-primary hover:text-white transition-all shadow-lg"
                >
